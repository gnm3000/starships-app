...
def jwt_token(client):
    ...
    print(f"Login Response: {response.json_body}")  # Debugging line
    return response.json_body["token"]


def test_get_starships_not_auth(client):
    response = client.http.get("/starships")
    assert response.status_code == 401  # Adjust if necessary based on your application
...
def test_get_starship(client, jwt_token):
    ...
    print(f"Starships Response: {response.json_body}")  # Debugging line
    assert response.status_code == 200
